...
#include "tests/Examples/openfhe/ckks/mtp_jkls_batch_matmul/mtp_jkls_batch_matmul_lib.h"

namespace mlir {
namespace heir {
namespace openfhe {

TEST(MtpJklsBatchMatmulOpenFHETest, TwoPackedTiles) {
  ...
  auto cryptoContext = mtp_jkls_batch_matmul__generate_crypto_context();
  auto keyPair = cryptoContext->KeyGen();
  auto publicKey = keyPair.publicKey;
  auto secretKey = keyPair.secretKey;
  cryptoContext = mtp_jkls_batch_matmul__configure_crypto_context(
  ...
  auto lhsEncrypted =
      mtp_jkls_batch_matmul__encrypt__arg0(cryptoContext, lhs, publicKey);
  auto rhsEncrypted =
      mtp_jkls_batch_matmul__encrypt__arg1(cryptoContext, rhs, publicKey);
  ...
  auto resultEncrypted =
      mtp_jkls_batch_matmul(cryptoContext, lhsEncrypted, rhsEncrypted);
  ...
  auto result = mtp_jkls_batch_matmul__decrypt__result0(

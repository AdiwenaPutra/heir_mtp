...
#include "tests/Examples/openfhe/ckks/mtp_jkls_batch_matmul/mtp_jkls_batch_matmul_auto_multi_ct_even_lib.h"
...
// MtpJklsBatchMatmulOpenFHETest.FourTilesAcrossTwoCiphertexts in
// mtp_jkls_batch_matmul_multi_ct_test.cpp — same numeric example, but the
// source MLIR has no secret.kernel or tensor_ext.layout; MTP-JKLS is selected
// automatically, with balanced packing evenly filling both ciphertexts.
TEST(MtpJklsBatchMatmulAutoOpenFHETest, FourTilesAcrossTwoCiphertextsAutoSelected) {
  ...
  auto cryptoContext =
      mtp_auto_multi_ct_even__generate_crypto_context();
  auto keyPair = cryptoContext->KeyGen();
  auto publicKey = keyPair.publicKey;
  auto secretKey = keyPair.secretKey;
  cryptoContext = mtp_auto_multi_ct_even__configure_crypto_context(
  ...
  auto lhsEncrypted = mtp_auto_multi_ct_even__encrypt__arg0(
      cryptoContext, lhs, publicKey);
  auto rhsEncrypted = mtp_auto_multi_ct_even__encrypt__arg1(
  ...
  auto resultEncrypted = mtp_auto_multi_ct_even(
  ...
  auto result = mtp_auto_multi_ct_even__decrypt__result0(

// RUN: heir-opt --convert-to-ciphertext-semantics=min-slot-count=8 %s | FileCheck %s
...
#mtp = #tensor_ext.layout<"{ [p, l, d] -> [ct, slot] : ct = 0 and slot - 4 * d - 2 * p - l = 0 and 0 <= p <= 1 and 0 <= l <= 1 and 0 <= d <= 1 and 0 <= slot <= 7 }">
